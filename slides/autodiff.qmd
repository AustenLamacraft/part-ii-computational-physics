...
- Popular choice is _one hot encoding_. $\mathbf{y}_i$ is $N_L$ dimensional vector ($N_L$ is number of labels)

- Label $n$ encoded as $(0,0,\ldots, 1, \ldots, 0)$, with $1$ in $n\text{th}$ place 
...
- Not a trvial task: GPT and other LLMs > _1 trillion_ parameters!
...
- This is [numerical differentiation](https://en.wikipedia.org/wiki/Numerical_differentiation)
...
- Numerical differentiation would require us to vary each of the parameters in the network separately (hundreds of trillions of parameters)
