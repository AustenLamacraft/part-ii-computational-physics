...
Students may start their project work once the project list is published by 20th February. Click on the [GitHub Classroom link](https://classroom.github.com/a/EubtKrnp) to accept the project as an assignment. The deadline for submission of the project report is __16:00 on the first Monday of Full Easter term (4th May 2026)__. Submissions made after that time will not be marked (see @sec-late-submission). I suggest you proceed _as if the deadline is 16.00 on Friday 1st May_, and use the final weekend if you really need to.
...
Please see [Keeping Laboratory Notes and Writing Formal Reports](https://www-teach.phy.cam.ac.uk/dms/dms_getFile.php?node=54292) for further guidance. There is no prescription on the length of your report, except that it should provide a comprehensive account of the work you have done. For a guide to the presentation of Python code, see [PEP 8](https://peps.python.org/pep-0008/) and [this explanation](https://realpython.com/python-pep8/).

If you generate large data files that you wish to include, you may find you run into GitHub's [restrictions on file sizes](https://docs.github.com/en/repositories/working-with-files/managing-large-files/about-large-files-on-github). GitHub blocks files larger than 100 MiB. For larger files you can use [Git Large File Storage](https://docs.github.com/en/repositories/working-with-files/managing-large-files/about-git-large-file-storage). The free tier gives you 2 GB, which should be enough.


# Submission {#sec-submission}

The submission instructions are the same as for the [exercises](exercises.qmd). Click on the [GitHub Classroom link](https://classroom.github.com/a/EubtKrnp) to accept the project as an assignment. Please __change the name of your repo to `<project-name>-<CRSid>`__ for ease of marking. 

You should take advantage of the fact that you are using GitHub to __regularly commit to your project repository__ and __push the commits to GitHub__. That way there is no danger of you accidentally missing the deadline: your last commit to the repository on GitHub will constitute your submission. 

## Late submission and extensions {#sec-late-submission}

See the [NST rules on late submission](https://universityofcambridgecloud.sharepoint.com/sites/NST_NaturalSciencesTripos/SitePages/Coursework%20Extensions.aspx): _"Students are required to self-certify before a submission deadline or at the point of submission of the work. Retrospective requests will not be accepted. Where a student does not self-certify and does not submit their coursework by the original submission date, zero marks will be awarded. Where a student has self-certified and does not submit their work by the revised submission date, zero marks will be awarded, unless a further extension has been granted by the department or EAMC"_. The form for submitting a self-certified extension is [here](https://forms.office.com/pages/responsepage.aspx?id=RQSlSfq9eUut41R7TzmG6Y64hS-YY0NHhP2hmw9lbpdUNFdLU01PRTFXVURORUhBVllUS0QwSkxBRS4u&route=shorturl). __Do not contact me directly with extension requests__.

Late submissions are made in exactly the same way as ordinary submissions. __Your last commit to your repo before the (extended) deadline will be your submission__.
...
You will need do some research and reading before starting, so follow the links and references[^2] given, or find your own. _But read pragmatically_: look for the algorithm description or pseudo-code and start thinking about how you will translate it into your own code.
...
Study the performance of cluster algorithms for statistical mechanics models such as the Ising model and hard disks — as described in [these lectures](https://hef.ru.nl/~tbudd/mct/lectures/cluster_algorithms.html) or @krauth2006statistical — and compare them with the Metropolis algorithm. You should analyze the autocorrelation time of the algorithms and the issue of critical slowing down. 
...
## Model Synthesis / Wavefunction Collapse

Despite the name, this is not about quantum mechanics but rather constraint-solving or [texture synthesis](https://en.wikipedia.org/wiki/Texture_synthesis): think artificial landscapes in computer games. Model synthesis and wavefunction collapse are in fact variants of the same basic idea, as explained on [this wikipedia page](https://en.wikipedia.org/wiki/Model_synthesis) (see also [this GitHub repo](https://github.com/mxgmn/WaveFunctionCollapse)). There are potential connections with statistical physics: see [this Twitter thread](https://twitter.com/ExUtumno/status/1119996186663030784). When does the algorithm fail to find a solution?
