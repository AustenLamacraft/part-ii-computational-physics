...
One geometrical interpretation of the SVD is as follows. The columns of $V$ define an orthonormal basis $\mathbf{v}_i\in \mathbb{C}^n$ ($i=1,\ldots n$). Likewise $U$ defines a basis $\mathbf{u}_i\in \mathbb{C}^m$ $i=1,\ldots m$. If we act on $\mathbf{v}_i$ with $M$ (to the left) we get $\lambda_i \mathbf{u}_i$. 
