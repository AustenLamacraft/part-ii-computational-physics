...
What is the complexity of computing the Fourier transform? @eq-dft may be regarded as matrix-vector multiplication, where $f_j$ are the $N$ components of a vector and the matrix has elements $Q_{n j}\equiv e^{-i\eta_n j}$, so tha the DFT is written
$$
